...
#split files  ... firstname,nametype = fullname.split(“-”)
#newpath = directory/firstname/
#os.mkdir(newpath)
...
def move_files(path=os.getcwd()):
	files = os.listdir(path)
	existing_files={}
	for file in files: 
		if file.endswith('.edf'):
			firstname,nametype = fullname.split(“-”)
			newpath = os.path.join(path, firstname)
			if existing_files.get(firstname):
				shutil.copyfile(os.path.join(path, file), os.path.join(newpath, file))
			else:
				os.mkdir(newpath)
				existing_files[newpath] = True
				shutil.copyfile(os.path.join(path, file), os.path.join(newpath, file))
